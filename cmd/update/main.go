...
func getBaseDirectory() string {
	...
	workd, _ := os.Getwd()
	searchPaths := []string{
		filepath.Join(workd, "data", "ieee"),
		filepath.Join(workd, "..", "data", "ieee"),
		filepath.Join(workd, "..", "..", "data", "ieee"),
		filepath.Join(filepath.Dir(execd), "data", "ieee"),
		filepath.Join(filepath.Dir(execd), "..", "data", "ieee"),
		filepath.Join(filepath.Dir(execd), "..", "..", "data", "ieee"),
	}
	...
	for _, p := range searchPaths {
		...
		if _, err := os.Stat(p); err == nil {
			based, _ := filepath.Abs(filepath.Join(p, "..", ".."))
			log.Printf("Found data directory at %s, using base directory %s", p, based)
			return based
		}
	}
...
func writeOUIBin(info *MACUpdate) {
	...
	for prefix, entries := range info.data {
		...
		mask := "24"
		if len(addrMaskBits) == 2 {
			mask = addrMaskBits[1]
		}

		maskInt, merr := strconv.Atoi(mask)
		if merr != nil {
			log.Fatalf("error parsing mask %#v: %s", prefix, merr)
		}

		addr, perr := mactracker.ParseMAC(addrMaskBits[0])
		if perr != nil {
		...
		lastAddress := ""
		for _, entry := range entries {
			if len(firstAdded) == 0 && entry.Type == "add" {
